...
const ClassSetup = () => {
  ...
  useEffect(() => {
    ...
    setMediums(allMediums.filter(m => m.shift_id === Number(formData.shift) && m.status === "Active"));
    setFormData(prev => ({ ...prev, medium: "", edu_level: "", department: "" }));
  ...
  const handleSubmit = e => {
    ...
    if (editId) {
      setClasses(prev => prev.map(c => (c.id === editId ? { ...formData, id: editId } : c)));
    } else {
      const newId = classes.length ? Math.max(...classes.map(c => c.id)) + 1 : 1;
      setClasses(prev => [...prev, { ...formData, id: newId, class_code: generatedCode }]);
      setGeneratedCode((Number(generatedCode) + 1).toString().padStart(4, "0"));
    }

    setEditId(null);
    setFormData({
      shift: "", medium: "", edu_level: "", department: "",
      class_name: "", class_code: "", ordering: "",
      roll_identifier: "", status: "Active",
    });
  };
  ...
  // ---------------- Helpers ----------------
  const getName = (arr, id, key) => arr.find(x => x.id === Number(id))?.[key] || "-";
        ...
        <form className="form-layout" onSubmit={handleSubmit}>
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <select name="edu_level" value={formData.edu_level} onChange={handleChange}>
                <option value="">Select Education Level</option>
                {filteredEducationLevels.map(e => <option key={e.id} value={e.id}>{e.title}</option>)}
              </select>
            </div>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input name="class_name" value={formData.class_name} onChange={handleChange} placeholder="Enter Class Name" />
            </div>

            <div className="form-group">
              <label>Class Code</label>
              <input value={editId ? formData.class_code : generatedCode} readOnly placeholder="Class Code" />
            </div>

            <div className="form-group">
              <label>Roll Identifier <span className="required">*</span></label>
              <input name="roll_identifier" value={formData.roll_identifier} onChange={handleChange} placeholder="Enter Roll Identifier" />
            </div>

            <div className="form-group">
              <label>Ordering</label>
              <input name="ordering" value={formData.ordering} onChange={handleChange} placeholder="Enter Order Number" />
            </div>
