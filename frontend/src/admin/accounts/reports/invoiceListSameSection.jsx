...
import "../../../assets/styles/admin/accounts/reports/invoiceListSameSection.css";

// Sample invoices with multiple entries for section demo
...
const InvoiceListSameSection = () => {
  ...
  const handlePrint = () => {
    ...
    doc.html(invoiceRef.current, {
      callback: function (pdf) {
        pdf.save("InvoiceListSameSection.pdf");
      },
      margin: [20, 20, 20, 20],
      autoPaging: "text",
      x: 10,
  ...
  const handleExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "InvoiceListSameSection");
    XLSX.writeFile(wb, "InvoiceListSameSection.xlsx");
  };
  ...
  // Group by same section (shift + medium + educationLevel + department + currentClass + section)
  const groupBySection = (invoices) => {
    const grouped = {};
    invoices.forEach((inv) => {
      const key = `${inv.shift}-${inv.medium}-${inv.educationLevel}-${inv.department}-${inv.currentClass}-${inv.section}`;
  ...
  const groupedInvoices = groupBySection(searchResult);

  return (
    <div className="invoice-list-details-page">
      <h2>Invoice List - Same Section</h2>
      ...
      {searchResult.length > 0 && (
        <div className="results-section" ref={invoiceRef}>
          <div className="invoice-header">
            ...
            <h4>Invoices Grouped by Same Section</h4>
          ...
          {Object.keys(groupedInvoices).map((key, idx) => {
            const invoices = groupedInvoices[key];
            const totalAmount = invoices.reduce((sum, inv) => sum + inv.total, 0);
            return (
              <div key={idx} className="student-group">
                <h4>Section Group: {key} | Total Count: {invoices.length}</h4>
                <table>
                  <thead>
                    <tr>
                      <th>SL</th>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td colSpan="23">
                        <strong>
                          Section Total Count: {invoices.length} | Section Total Amount: {totalAmount.toFixed(2)} TK
                        </strong>
                      </td>
                    </tr>
                  </tbody>
...
export default InvoiceListSameSection;
