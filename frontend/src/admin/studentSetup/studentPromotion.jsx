...
import "../../assets/styles/admin/studentSetup/studentPromotion.css";
...
const StudentPromotion = () => {
  ...
  const handlePromote = () => {
    ...
    alert(
      `Promoted ${selectedStudents.length} students to ${filters.promoteClass}-${filters.promoteSection}`
    );
      ...
      {showTable && (
        <div className="additional-filters">
          <label>
            <input
              type="checkbox"
              checked={filters.prevCourseAdvising}
              onChange={(e) => setFilters({ ...filters, prevCourseAdvising: e.target.checked })}
            />
            With Previous Course Advising
          </label>

          <label>
            <input
              type="checkbox"
              checked={filters.prevClassRoll}
              onChange={(e) => setFilters({ ...filters, prevClassRoll: e.target.checked })}
            />
            With Previous Class Roll
          </label>
          ...
          <div className="filters promote-to">
            {[
              { label: "Shift", key: "promoteShift", options: ["Day", "Morning"] },
              { label: "Medium", key: "promoteMedium", options: ["Bangla", "English"] },
              { label: "Edu Level", key: "promoteEduLevel", options: ["", "Higher Secondary", "Secondary"] },
              { label: "Department", key: "promoteDepartment", options: ["", "Science", "Arts"] },
              { label: "Class", key: "promoteClass", options: ["", "HSC-Science", "HSC-Arts"] },
              { label: "Section", key: "promoteSection", options: ["", "2nd Year", "3rd Year"] },
              { label: "Session", key: "promoteSession", options: ["", "2026-2027", "2027-2028"] },
            ].map((f) => (
              <label key={f.key}>
                {f.label}:
                <select
